...
const Onboarding = ({ onComplete }) => {
  ...
  const renderOnboarding = () => (
    <div className="fade-in flex flex-col items-center text-center w-full">
      <div className="flex items-center justify-center bg-surface w-20 h-20 rounded-2xl mb-5 text-primary shadow-md" style={{ border: '1px solid rgba(255,255,255,0.8)' }}>
        <ShieldCheck size={44} strokeWidth={1.5} />
      </div>
      <h1 className="text-3xl font-bold mb-1">TrueVoice</h1>
      <p className="text-base text-secondary mb-6">Protecting you from AI voice scams</p>
        ...
        <form onSubmit={handleSendOtp}>
          <div className="input-group mb-4">
            ...
            <div className="relative flex items-center">
              ...
              <input type="text" className="input-field" style={{ paddingLeft: '2.5rem' }} placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} required />
            </div>
          </div>
          <div className="input-group mb-6">
            ...
            <div className="relative flex items-center">
              ...
              <input type="tel" className="input-field" style={{ paddingLeft: '2.5rem' }} placeholder="+91 99999 99999" value={phone} onChange={(e) => setPhone(e.target.value)} required />
            </div>
          </div>
  ...
  const renderOTP = () => (
    <div className="fade-in flex flex-col items-center text-center w-full">
      <h1 className="text-2xl font-bold mb-1">Verify Phone</h1>
      <p className="text-secondary mb-6">We sent a code to {phone}</p>

      <div className="card w-full">
        <form onSubmit={handleVerifyOtp}>
          <div className="input-group mb-6">
            <label className="input-label text-center">Enter the 4-digit code</label>
            <input type="text" className="input-field text-center text-xl tracking-widest" maxLength="4" value={otp} onChange={(e) => setOtp(e.target.value)} required />
          </div>
          {otpHint && (
          ...
          <button type="submit" className="btn btn-primary w-full">Verify &amp; Continue</button>
        </form>
      </div>
    </div>
  );
  ...
  const renderVoiceRegistration = () => (
    <div className="fade-in flex flex-col items-center text-center w-full">
      ...
      <p className="text-base text-secondary mb-6 max-w-xs">We'll create a unique, invisible watermark for your voice to protect you from AI cloning.</p>
      ...
      <div className="card w-full text-center mb-4">
        ...
        <button onClick={handleRecordVoice} disabled={isRecording} className={`btn ${isRecording ? 'btn-outline border-danger text-danger' : 'btn-primary'} w-full flex items-center justify-center gap-2`}>
          {isRecording ? <div className="animate-pulse flex items-center gap-2"><Mic size={20} /> Recording (5s)...</div> : <><Mic size={20} /> Tap to Record</>}
        </button>
      </div>

      <div className="bg-surface p-3 rounded-md flex items-start gap-3 text-sm w-full">
        <Lock className="text-success mt-0.5 shrink-0" size={16} />
        <div className="text-left">
          <strong className="text-sm block leading-tight mb-0.5">Privacy First</strong>
          <p className="mb-0 text-xs leading-tight text-secondary">We do not store your raw audio. It is converted to an encrypted mathematical hash on your device.</p>
        </div>
      </div>
    </div>
  );
  ...
  const renderAnalyzing = () => (
    <div className="fade-in flex flex-col items-center text-center w-full">
      ...
      <p className="text-secondary max-w-[240px] mb-6">Generating high-frequency cryptographic watermark</p>
      <div className="w-56 bg-surface h-2 rounded-full overflow-hidden">
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--background, #f0f4ff)', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: '420px', background: '#fff', borderRadius: '24px', padding: '2.5rem 2rem', boxShadow: '0 20px 60px rgba(0,0,0,0.10)' }}>
